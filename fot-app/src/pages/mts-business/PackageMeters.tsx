...
/**
 * Остатки пакетов номера (минуты/SMS/интернет). ValidityInfo отдаёт только
 * остаток (CurrentValue счётчиков Remainder) без квоты — тогда показываем
 * «осталось X» без «из Y» и без полосы заполнения. Дедупликация по имени
 * счётчика + строке значения (МТС иногда шлёт дубли). Пустой набор данных →
 * null (секцию не показываем). Переиспользуется в админ-дровере «Абоненты»
 * и в ЛК «Моя SIM».
 */
