...
const ACTION_LABELS: Record<string, string> = {
  ...
  BLACKLIST_ADDED: 'Внесён в чёрный список',
  BLACKLIST_REMOVED: 'Снят из чёрного списка',
  BLACKLIST_ENFORCED: 'Блокировка по чёрному списку применена',
  BLACKLIST_BLOCKED_ATTEMPT: 'Попытка заблокированного действия',
  BLACKLIST_MEMO_ADDED: 'Приложена служебная записка',
  BLACKLIST_MEMO_REMOVED: 'Удалена служебная записка',
  ...
  EXPORT_EMPLOYEES_VIEW: 'Выгрузка таблицы сотрудников',
  UPDATE_STAFF_COMMENT: 'Комментарий к сотруднику изменён',
  ...
  UPDATE_LEAVE_REQUEST_REASON: 'Текст заявления изменён',
  UPDATE_LEAVE_REQUEST_CORRECTION_HOURS: 'Часы корректировки изменены',
  UPDATE_LEAVE_REQUEST_TYPE: 'Категория заявления изменена',
};
...
const ACTION_GROUPS: Record<string, string[]> = {
  ...
  'Пользователи': ['USER_APPROVED', 'USER_REJECTED', 'USER_DELETED', 'EMAIL_CONFIRMED', 'POSITION_CHANGED', 'ROLE_CHANGED', 'NAME_CHANGED', 'USER_DEPARTMENT_ACCESS_CHANGED', 'CHAT_INBOUND_MODE_CHANGED', 'ORG_ASSIGNED', 'BLACKLIST_ADDED', 'BLACKLIST_REMOVED', 'BLACKLIST_ENFORCED', 'BLACKLIST_BLOCKED_ATTEMPT', 'BLACKLIST_MEMO_ADDED', 'BLACKLIST_MEMO_REMOVED'],
  'Сотрудники': ['VIEW_EMPLOYEES', 'EXPORT_EMPLOYEES_VIEW', 'UPDATE_STAFF_COMMENT', 'CREATE_EMPLOYEE', 'UPDATE_EMPLOYEE', 'DELETE_EMPLOYEE', 'DELETE_ALL_EMPLOYEES', 'ARCHIVE_EMPLOYEE', 'RESTORE_EMPLOYEE', 'FIRE_EMPLOYEE', 'REHIRE_EMPLOYEE', 'MOVE_EMPLOYEE_DEPARTMENT', 'IMPORT_EMPLOYEES', 'ENRICH_EMPLOYEES', 'ENRICH_EMPLOYEES_CONTACTS'],
  ...
  'Заявления': ['UPDATE_LEAVE_REQUEST_REASON', 'UPDATE_LEAVE_REQUEST_CORRECTION_HOURS', 'UPDATE_LEAVE_REQUEST_TYPE'],
};
...
function actionClass(action: string): string {
  if (['LOGIN_FAILED', '2FA_FAILED', 'USER_REJECTED', 'BLACKLIST_ADDED', 'BLACKLIST_BLOCKED_ATTEMPT', 'DELETE_EMPLOYEE', 'DELETE_ALL_EMPLOYEES', 'FIRE_EMPLOYEE', 'CLEAR_SKUD', 'CLEAR_STRUCTURE'].includes(action)) return styles.tagDanger;
