...
import { Coffee, LogIn, LogOut, Timer, XCircle } from 'lucide-react';
...
import styles from './LeaveRequestSkudEvents.module.css';

interface ILeaveRequestSkudEventsProps {
  employeeId: number;
  date: string;
}

const EMPTY_POINTS = new Set<string>();

const formatTime = (time: string): string => time.slice(0, 5);

/**
 * Проходы СКУД за день корректировки — inline-блок в раскрытии строки заявления.
 * Настройки точек доступа общие для всех раскрытых строк (один запрос на страницу),
 * события — свои для каждой пары сотрудник+дата.
 */
export const LeaveRequestSkudEvents: FC<ILeaveRequestSkudEventsProps> = ({ employeeId, date }) => {
  const settingsQuery = useQuery({
    queryKey: ['skud-access-point-settings'],
    queryFn: () => skudService.getAccessPointSettings(),
    staleTime: 5 * 60_000,
  });

  const internalPoints = useMemo(
    () => (settingsQuery.data
      ? new Set(settingsQuery.data.filter(s => s.is_internal).map(s => s.access_point_name.trim()))
      : EMPTY_POINTS),
    [settingsQuery.data],
  );
  ...
  return (
    <div className={styles.inline} aria-label="Проходы СКУД">
      <span className={styles.caption}>Проходы СКУД</span>
