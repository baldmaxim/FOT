...
import { Ban, Check, ChevronDown, ChevronUp, Clock, DoorOpen, Paperclip, X } from 'lucide-react';
import {
  REQUEST_TYPE_LABELS,
...
export interface ILeaveRequestRowProps {
  ...
  /** Открыта ли панель событий СКУД именно по этой корректировке. */
...
export const LeaveRequestRow: FC<ILeaveRequestRowProps> = ({
  ...
  return (
    <li
      className={`lrm-item${isEventsActive ? ' lrm-item--active' : ''}${expanded ? ' lrm-item--expanded' : ''}`}
    ...
    >
        ...
        <div className="lrm-item-actions" onClick={stop}>
          ...
          {hasSkudEvents && (
            <button
              ...
              title="Проходы СКУД"
              aria-label="Проходы СКУД"
              aria-pressed={isEventsActive}
              onClick={() => onOpenEvents(r)}
            >
              <DoorOpen size={14} />
            </button>
          )}
      ...
      {expanded && (
        <div className="lrm-item-details" id={detailsId} onClick={stop}>
