...
const shouldBypassHttpCache = (endpoint: string, method = 'GET'): boolean => {
  ...
  return path === '/skud/presence'
    ...
    || path === '/admin/data-api/keys'
    // Лог запросов ключа: 1С дёргает API и лог смотрят сразу, кэш тут только мешает.
    || /^\/admin\/data-api\/keys\/[^/]+\/logs$/.test(path)
    // Списки пользователей админки: после удаления/одобрения/смены роли рефетч
    // обязан идти на сервер. Иначе HTTP-кэш max-age=30 отдавал тело, снятое ДО
    // мутации — удалённый пользователь возвращался в список на ~30 сек (вместе с
    // бейджем «Все пользователи (N)»), и админ жал «Удалить» повторно, хотя в БД
    // он уже удалён с первого раза.
    || path === '/admin/users'
    || path === '/admin/users/pending'
    || path === '/admin/users/password-reset-requests'
    ...
    || path.startsWith('/sigur/admin/')
    // Вводный инструктаж («Управление кадрами»): после PATCH даты (проставили или
    // сняли крестиком) refetch обязан идти на сервер. Иначе HTTP-кэш max-age=30
    // отдавал тело, снятое ДО PATCH — дата «отскакивала» назад, крестик визуально
    // не срабатывал, и ОТиТБ жал по нескольку раз с перезагрузкой страницы,
    ...
    // Чеки за патент: после удаления чека админом и после загрузки чека из ЛК рабочего
    // список обязан обновиться сразу. max-age=30 отдавал тело, снятое ДО мутации —
