...
- **Z-index Header'а**: `Header.module.css` → `z-index: 95`. Header создаёт stacking context; все модалки/дропдауны внутри него (notifications, меню) должны быть выше, чем sticky-заголовки табеля (z-40...90). Иерархия: Sidebar (100) > Header (95) > Content. НЕ менять Header z-index без проверки, что dropdowns остаются на месте и выше sticky-рядов.
