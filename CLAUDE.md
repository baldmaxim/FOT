...
- **FK на пользователя**: политика из миграций 284/285 — владение (подписки, доступы, участие в чатах) `ON DELETE CASCADE`; авторство (кто согласовал/создал/загрузил) в колонке `NOT NULL` — `ON DELETE SET DEFAULT` на надгробный профиль «Удалённый пользователь» (`config/system-users.ts`), в колонке с `NULL` — `ON DELETE SET NULL` **без** `DEFAULT` (иначе `INSERT`, где колонка опущена, запишет надгробие вместо `NULL` — так на проде появились «призрачные» авторы у заявки). Новая колонка со ссылкой на `user_profiles`/`app_auth.users` обязана попасть в политику, иначе удаление учётки падает; стережёт `npm run audit:user-fk` (вызывается в `deploy-server.sh` после миграций).
...
### iOS 26 Safari (Liquid Glass)

С iOS 26 Safari рисует страницу edge-to-edge: статус-бар сверху и панель вкладок снизу — полупрозрачное «стекло», которое **размывает всё, что под ним**. В портрете это видно как размытая полоса над шапкой.

- `<meta name="theme-color">` Safari 26 игнорирует. Цвет хрома = `background-color` fixed/sticky-элемента у края (≤4px от верха, ширина ≥80%), иначе — фон `body`. Семплинг один раз при загрузке: смена темы через JS хром не перекрашивает — мириться.
- Баг iOS 26.0–26.1 (WebKit #302272): fixed/sticky **не рисуются в зоне статус-бара** — туда протекает обычный контент, и стекло его размывает. Поэтому шапки лейаутов — `position: relative` внутри нескроллящегося `.main`, скролл — в `.content` (`overflow-y: auto`), а не в `body`. Зону safe-area закрывает сама шапка (`padding-top: env(safe-area-inset-top)`), не sticky-элемент.
- `html` и `body` — явный фон через токен, не transparent.
- Скрытые fixed-оверлеи прятать только `display: none`/unmount, не `opacity: 0` (иначе семплируются в цвет хрома).
- Нижний край скролл-контейнеров — `padding-bottom: env(safe-area-inset-bottom)`, иначе последняя строка под панелью вкладок.
- Проверять в реальном Safari iOS 26 в портрете; DevTools стекло не эмулирует.
