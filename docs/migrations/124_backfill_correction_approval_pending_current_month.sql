-- 124: Бэкфилл очереди "Согласование выходных дней" за текущий месяц.
-- До коммита e7fcc79 при одобрении руководителем заявления-time_correction
-- attendance_adjustments создавались с approval_status='auto_approved' даже
-- если день выходной у сотрудника из whitelist-отдела. Эта миграция переводит
-- такие исторические записи (только текущий месяц МСК) в 'pending', чтобы они
-- появились на странице /approvals.
--
-- Правило отбора грубое: DOW IN (0,6) (Sat/Sun). Cycle-графики и праздничные
-- субботы доводятся вручную через UI: /approvals -> "Настройки согласования"
-- -> Сохранить (запускает reapproveAdjustmentsForRange по корректной логике).
