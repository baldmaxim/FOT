...
interface IWeekendCapableSchedule {
...
const expectedWeekendCount = (
  schedule: { expected_saturdays_per_month: number; expected_sundays_per_month: number },
  dow: WeekendDow,
): number => dow === 6 ? schedule.expected_saturdays_per_month : schedule.expected_sundays_per_month;

/**
 * Зачёт обязательного выходного: первые expected_{saturdays|sundays}_per_month
 * непраздничных суббот/воскресений месяца со статусом work/remote →
 * auto_approved. Праздничная Сб/Вс (по производственному календарю) в норму не
 * зачитывается. Применяется к любому графику с expected_{...}_per_month > 0
 * (в т.ч. cycle), не только legacy 5+2.
 */
export const isMandatoryWeekendSlotAvailable = (
  schedule: IWeekendCapableSchedule,
  workDate: string,
  dateObj: Date,
  ...
  if (dateObj.getDay() !== dow) return false;
  const expected = expectedWeekendCount(schedule, dow);
...
/**
 * Праздничная календарная дата с точки зрения графика: mandatory_holidays всегда,
 * обычные holidays — только если график их уважает (respects_holidays). Нужна, чтобы
 * исключить праздничные субботы из кандидатов обязательного слота (праздничная суббота
 * в квоту не входит). isHolidayWeekend/isHolidayOnWorkday-ветка в квоте НЕ используются:
 * квота считается строго по субботам (dow=6).
 ...
 * за ВЕСЬ месяц (status work/remote, approval auto_approved/approved). Не
 * ограничено запрошенным диапазоном табеля — обязательные выходные считаются
 * помесячно. Один SELECT, разнос по dow на клиенте.
 */
async function loadAcceptedWeekendDaysForMonth(
  employeeIds: number[],
  year: number,
  mon: number,
  ...
  if (employeeIds.length === 0) return { saturdays, sundays };
...
/**
 * Кандидаты обязательного субботнего слота: для каждого сотрудника — ДЕДУПЛИЦИРОВАННЫЙ
 * Set ISO-суббот месяца, в которые он фактически работал. Используется резолвером и
 * reapprove для определения «первой отработанной субботы» (usedBefore < expected).
 *
 * Отличия от loadAcceptedWeekendDaysForMonth (та осталась для нормы/недобора грида):
 *  - включает pending (иначе накопившиеся pending не повысить при пересчёте);
 *  - фильтрует нулевые часы (work с hours_override=0 = «не работал» → слот не занимает);
 *  - без «первых N» slice — возвращает ВСЕ отработанные субботы, выбор N делает usedBefore;
...
async function loadWorkedSaturdaysForQuota(
  ...
  const add = (empId: number, iso: string): void => {
    const dateObj = new Date(`${iso}T00:00:00`);
    if (dateObj.getDay() !== 6) return; // только субботы
    const sched = schedulesMap.get(empId);
    if (sched && isHolidaySaturday(iso, sched, calendar)) return; // праздничная суббота не в квоте
  ...
  for (const row of adjRows) {
    add(Number(row.employee_id), String(row.work_date).slice(0, 10));
  }
  ...
  const skudRows = await queryWith<{ employee_id: number | string; date: string }>(
    ...
    `SELECT sds.employee_id, sds.date
       FROM skud_daily_summary sds
      WHERE sds.employee_id = ANY($1::int[])
        AND sds.date >= $2 AND sds.date <= $3
        AND sds.is_present = true
        AND NOT EXISTS (
          SELECT 1 FROM attendance_adjustments aa
           WHERE aa.employee_id = sds.employee_id AND aa.work_date = sds.date
        )`,
    [employeeIds, monthStart, monthEnd],
  );
  for (const row of skudRows) {
    add(Number(row.employee_id), String(row.date).slice(0, 10));
  }
...
export async function computeReapprovalTransitions(
  ...
  // Кандидаты обязательного субботнего слота — кэш по (сотрудник, месяц), грузится на
  // весь месяц один раз (зеркалит квоту resolveAdjustmentApprovalStatus). Набор строится
  // из ИСХОДНОГО состояния БД до апдейтов, поэтому при пересчёте пары pending первая (по
  // дате) станет auto, вторая останется pending.
  const workedSatCache = new Map<string, Set<string>>();
  const getWorkedSaturdays = async (
    empId: number,
    dateObj: Date,
    schedule: IResolvedSchedule,
  ): Promise<Set<string>> => {
  ...
  const computeTarget = async (row: Row): Promise<'auto_approved' | 'pending'> => {
    ...
    if (WORKED_STATUSES_FOR_APPROVAL.has(status) && dateObj.getDay() === 6
      ...
      const usedBefore = [...worked].filter(d => d < workDate).length;
      return isMandatoryWeekendSlotAvailable(schedule, workDate, dateObj, calendar, usedBefore, 6)
...
export const timesheetController = {
  ...
  async getAll(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      // ─── Обязательные выходные (SchedulesPage → «Обязательные выходные») ───
      // Проверка ПОМЕСЯЧНАЯ: норма месяца включает N Сб + M Вс, отработанные
      ...
      const applyMandatoryWeekendShortfall = (dow: WeekendDow): void => {
        ...
        for (const empId of employeeIds) {
          const sched = schedulesMap.get(empId);
          if (!sched) continue;
          const expected = expectedWeekendCount(sched, dow);
          if (expected <= 0) continue;
          const empCutoff = cutoffByEmployeeId.get(empId) ?? null;
          const monthDays = listNonHolidayWeekendDays(year, mon, calendarMonth, sched.respects_holidays, dow);
          const inWindow = monthDays.filter(iso => !empCutoff || iso < empCutoff);
          if (inWindow.length === 0) continue;
          const last = inWindow[inWindow.length - 1];
          if (last >= todayStr) continue; // месяц по этим дням ещё не завершён
          if (endDate < last) continue;   // запрошенный период не закрывает месяц

          const required = Math.min(expected, inWindow.length);
          if (required <= 0) continue;

          const workedSet = acceptedByEmp.get(empId);
          let worked = 0;
          if (workedSet) {
          ...
          const dayHours = sched.work_hours || 0;
          const empStats = employeeStatsMap.get(empId);

          normHours += required * dayHours;
          if (empStats) empStats.norm_hours += required * dayHours;
          if (credited > 0) {
            actualHours += credited * dayHours;
            if (empStats) empStats.fact_hours += credited * dayHours;
          }
          if (shortfall > 0) deviations.absent += shortfall;
        }
      };
      applyMandatoryWeekendShortfall(6);
      applyMandatoryWeekendShortfall(0);
