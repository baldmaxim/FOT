...
describe('МТС Бизнес «Абоненты»: переадресация за абонента', () => {
  ...
  // Probe 22.07.2026: CallForwardingInfo на ЛС СУ-10 отвечает 421/3003 при 22
  // рабочих секциях из 24 — функция не обслуживается для договора. Такой отказ
  // не баг портала: отдаём понятную причину, а не общее «Ошибка включения».
  it('421/3003 (Foris) → 503 с подсказкой про услугу «Переадресация вызова»', async () => {
    const res = mockRes();
    catalog.changeCallForwarding.mockRejectedValue(new MtsBusinessApiError('Сервис Foris временно недоступен', 421, '3003'));
  ...
  it('403/1010 (не в подписке) → 409, а не 502', async () => {
    const res = mockRes();
    catalog.changeCallForwarding.mockRejectedValue(new MtsBusinessApiError('Запрос не авторизован', 403, '1010'));
