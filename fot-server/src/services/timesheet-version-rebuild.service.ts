// Аварийная пересборка официальных версий табеля. НЕ часть прикладного процесса.
//
// В штатной работе воркер простаивает: закрытый табель правится только через
// «Открыть → правки → Закрыть», и новая редакция создаётся там же, синхронно и
// атомарно. Ни один контроллер подачу на пересборку не помечает.
//
// Раньше было иначе: админ мог писать в закрытый период напрямую, такая правка
// ставила version_dirty, и воркер догонял версию за 30–90 секунд, а всё это время
// табель не выдавался в 1С. Привилегия снята, вместе с ней ушёл и штатный источник
// меток.
//
// Остаётся один сценарий — операторское восстановление после ручной правки БД:
// оператор помечает подачу через timesheet-version-maintenance.ts, и воркер собирает
// снимок заново. Синхронно этого делать нельзя: правка одного дня влечёт сборку
// табеля всего отдела.
//
// Холостой тик раз в 60 секунд идёт по partial-индексу version_dirty_at и стоит
// дёшево, поэтому воркер оставлен включённым.
