...
const { pgQueryOne, extractCompetencies } = vi.hoisted(() => ({
  pgQueryOne: vi.fn(),
  extractCompetencies: vi.fn(),
}));
...
vi.mock('./adaptive-testing-llm.service.js', () => ({
  ...
  adaptiveTestingLlmService: { generateQuestion: vi.fn(), evaluateTextAnswer: vi.fn(), extractCompetencies },
}));
...
describe('profileInputZod — файл скилла (.md) как единственный источник', () => {
  const base = {
  ...
  it('файл предельного размера проходит, на символ больше — отклоняется', () => {
    ...
    expect(() => profileInputZod.parse({ ...base, skillMd: atLimit, skillMdFilename: 'skill.md' })).not.toThrow();
    expect(() => profileInputZod.parse({ ...base, skillMd: `${atLimit}x`, skillMdFilename: 'skill.md' })).toThrow();
  });

  it('без файла — понятный отказ по-русски', () => {
    const res = profileInputZod.safeParse(base);
    expect(res.success).toBe(false);
    const message = res.success ? '' : res.error.issues.map(i => i.message).join('; ');
    expect(message).toMatch(/файл скилла/i);
    expect(message).not.toMatch(/Required|Array must contain/i);
  });

  it('файл из одних пробелов и переводов строк — отклоняется', () => {
  ...
  it('файл без имени — отклоняется', () => {
    expect(() => profileInputZod.parse({ ...base, skillMd: '# Методичка' })).toThrow(/имя загруженного файла/i);
  });

  // Ручной ввод убран: если старый клиент пришлёт эти поля, лучше явный отказ,
  // чем молча проигнорированные данные.
  it('обязанности и компетенции во входе больше не принимаются', () => {
    expect(() => profileInputZod.parse({
      ...base,
      skillMd: '# Методичка', skillMdFilename: 'skill.md',
      dutiesText: 'Оформляет заявки', competencies: [{ key: 'docs', name: 'Документы' }],
    })).toThrow(/Unrecognized key/i);
  });

  it('пустое название профиля — понятный текст', () => {
    const res = profileInputZod.safeParse({
      ...base, title: '', skillMd: '# Методичка', skillMdFilename: 'skill.md',
    });
    expect(res.success).toBe(false);
    const message = res.success ? '' : res.error.issues.map(i => i.message).join('; ');
...
describe('getAnswerReveal — ключ только по отвеченному вопросу', () => {
  beforeEach(() => {
    pgQueryOne.mockReset();
  });

  it('ответ есть — возвращает правильные варианты и рубрику', async () => {
    pgQueryOne.mockResolvedValueOnce({
      seq: 3,
      type: 'single',
      question_text: 'Вопрос?',
      options: [{ id: 'a', text: 'A' }, { id: 'b', text: 'Б' }],
    ...
    const res = await adaptiveTestingService.getAnswerReveal(2520, 'sess-1', 'q-1');

    expect(res.correctOptionIds).toEqual(['b']);
    expect(res.seq).toBe(3);
  });

  // Без строки ответа JOIN не даёт записи — ключ не должен утечь до выбора.
  it('вопрос не отвечен — 404, ключ не отдаётся', async () => {
    pgQueryOne.mockResolvedValueOnce(null);

    await expect(adaptiveTestingService.getAnswerReveal(2520, 'sess-1', 'q-1'))
      .rejects.toMatchObject({ httpStatus: 404, code: 'not_answered' });
  });

  // employee_id входит в WHERE: чужая сессия не отличима от неотвеченной.
  it('чужая сессия — 404', async () => {
    pgQueryOne.mockResolvedValueOnce(null);

    await expect(adaptiveTestingService.getAnswerReveal(9999, 'sess-1', 'q-1'))
      .rejects.toMatchObject({ httpStatus: 404 });
    const params = pgQueryOne.mock.calls[0][1] as unknown[];
    expect(params).toContain(9999);
  });

  it('текстовый вопрос — отдаётся рубрика вместо вариантов', async () => {
    pgQueryOne.mockResolvedValueOnce({
      seq: 4,
      type: 'text',
      ...
      answer: { type: 'text', text: 'Мой ответ' },
      score: 75,
      eval_state: 'evaluated',
    });

    const res = await adaptiveTestingService.getAnswerReveal(2520, 'sess-1', 'q-4');

    expect(res.rubric).toHaveLength(2);
    expect(res.options).toBeNull();
  });
});

describe('saveProfile — компетенции выводятся из файла', () => {
  const input = {
    orgDepartmentId: '11111111-1111-1111-1111-111111111111',
    positionId: null,
    title: 'Профиль отдела',
    isPublished: true,
    skillMd: '# Методичка отдела\n\nАрхитектура, работа с LLM, качество кода.',
    skillMdFilename: 'skill.md',
  };
  const EXTRACTED = [
    { key: 'architecture', name: 'Архитектура решений' },
    { key: 'llm', name: 'Работа с LLM' },
  ];

  beforeEach(() => {
    pgQueryOne.mockReset();
    extractCompetencies.mockReset();
  });

  it('новый профиль: темы берутся у LLM и сохраняются', async () => {
    extractCompetencies.mockResolvedValueOnce(EXTRACTED);
    pgQueryOne
      .mockResolvedValueOnce({ departmentName: 'ОЦТ', positionName: null })  // скоуп для промпта
      .mockResolvedValueOnce({ id: 'profile-1' });                            // INSERT

    const res = await adaptiveTestingService.saveProfile(input, 'user-1');

    expect(res).toMatchObject({ id: 'profile-1', competencies: EXTRACTED, competenciesFallback: false });
    expect(extractCompetencies).toHaveBeenCalledTimes(1);
  });

  it('файл не менялся — LLM не дёргается, темы остаются прежними', async () => {
    pgQueryOne
      .mockResolvedValueOnce({ skill_md: input.skillMd, competencies: EXTRACTED })  // текущий профиль
      .mockResolvedValueOnce({ id: 'profile-1' });                                   // UPDATE

    const res = await adaptiveTestingService.saveProfile(input, 'user-1', 'profile-1');

    expect(res.competencies).toEqual(EXTRACTED);
    expect(res.competenciesFallback).toBe(false);
    expect(extractCompetencies).not.toHaveBeenCalled();
  });

  it('файл заменили — темы пересобираются', async () => {
    extractCompetencies.mockResolvedValueOnce(EXTRACTED);
    pgQueryOne
      .mockResolvedValueOnce({ skill_md: '# Старая методичка', competencies: [{ key: 'old', name: 'Старое' }] })
      .mockResolvedValueOnce({ departmentName: 'ОЦТ', positionName: null })
  ...
  // Сбой провайдера не должен терять загруженную методичку.
  it('LLM недоступна — профиль сохраняется с запасной общей темой', async () => {
    extractCompetencies.mockRejectedValueOnce(new Error('LLM недоступна (503)'));
    pgQueryOne
      .mockResolvedValueOnce({ departmentName: 'ОЦТ', positionName: null })
      .mockResolvedValueOnce({ id: 'profile-1' });
    ...
    expect(res.competenciesFallback).toBe(true);
    expect(res.competencies).toHaveLength(1);
    expect(res.competencies[0].key).toBe('general');
  });
