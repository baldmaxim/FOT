...
describe('модельные предикаты (разделение по назначению)', () => {
  ...
  it('Gemini 3.1 Flash Lite: только OCR, не для тестирования', () => {
    expect(isKnownOpenRouterModel('google/gemini-3.1-flash-lite')).toBe(true);
    expect(isAllowedOcrModel('google/gemini-3.1-flash-lite')).toBe(true);
    expect(isAllowedTextModel('google/gemini-3.1-flash-lite')).toBe(false);
  });

  it('Gemini 3.1 Flash Lite есть в OCR-каталоге и отсутствует в каталоге тестирования', async () => {
    seedSettings({});
    const ocr = await settingsService.getOpenRouterConfig();
    expect(ocr.allowedModels.map(m => m.id)).toContain('google/gemini-3.1-flash-lite');

    seedSettings({});
    const adaptive = await settingsService.getAdaptiveTestingSettings();
    expect(adaptive.allowedModels.map(m => m.id)).not.toContain('google/gemini-3.1-flash-lite');
  });
