...
export const TRAVEL_SEGMENT_STATUSES: TravelSegmentStatus[] = [
  'auto_approved',
  'pending',
  'approved',
  'rejected',
...
const fetchExistingDecidedSegments = async ({
  employeeIds,
  startDate,
  endDate,
...
const mergeDecidedIntoSegments = (
  segments: ICalculatedTravelSegment[],
  decided: ITravelSegmentRow[],
): void => {
  if (decided.length === 0) return;

  const decidedByKey = new Map<string, ITravelSegmentRow>();
  for (const row of decided) {
    decidedByKey.set(segmentKey(row), row);
  }
  ...
  for (const segment of segments) {
    ...
    segment.approved_by = existing.approved_by;
    segment.approved_at = existing.approved_at;
    segment.approval_comment = existing.approval_comment;
  }
};
...
const summarizeSegmentsByDay = (segments: ICalculatedTravelSegment[]): Map<string, ITravelDaySummary> => {
  ...
  for (const segment of segments) {
    ...
    if (segment.status === 'pending') current.pendingSegmentsCount += 1;
...
export const listTravelSegments = async ({
  ...
  const loadPromise = (async () => {
    ...
    const approverIds = [...new Set(
      segments.map(segment => segment.approved_by).filter((value): value is string => !!value),
    )];
    const approverNames = await fetchApproverNames(approverIds);
    ...
    return segments.map(segment => {
      ...
      return {
        ...
        approved_by_name: segment.approved_by ? approverNames.get(segment.approved_by) || null : null,
        approved_at: segment.approved_at,
        approval_comment: segment.approval_comment,
...
const decorateSegmentRow = async (
  row: ITravelSegmentRow,
): Promise<ITravelSegmentListItem> => {
  const employeeRows = await fetchEmployeeRowsByIds([row.employee_id]);
  const employee = employeeRows[0] || null;
  const departmentMap = employee?.org_department_id
    ? await fetchDepartmentsMap([employee.org_department_id])
    : new Map<string, string>();
  const objectIds = [row.from_object_id, row.to_object_id].filter((value): value is string => !!value);
  const objects = objectIds.length > 0 ? await fetchTravelObjectsRaw() : [];
  const objectNameById = new Map<string, string>(objects.map(object => [object.id, object.name]));
  const approverNames = row.approved_by ? await fetchApproverNames([row.approved_by]) : new Map<string, string>();

  return {
    id: row.id,
    employee_id: row.employee_id,
    employee_name: employee?.full_name || `#${row.employee_id}`,
    department_id: employee?.org_department_id || null,
    department_name: employee?.org_department_id ? departmentMap.get(employee.org_department_id) || null : null,
    ...
    approved_by_name: row.approved_by ? approverNames.get(row.approved_by) || null : null,
...
export const listTravelSegmentsForEmployeeDay = async ({
  employeeId,
  workDate,
}: {
  employeeId: number;
  ...
  const objectIds = [...new Set(
    rows.flatMap(row => [row.from_object_id, row.to_object_id]).filter((value): value is string => !!value),
  )];
  const objects = objectIds.length > 0 ? await fetchTravelObjectsRaw() : [];
  const objectNameById = new Map<string, string>(objects.map(object => [object.id, object.name]));

  const approverIds = [...new Set(rows.map(row => row.approved_by).filter((value): value is string => !!value))];
  const approverNames = await fetchApproverNames(approverIds);
...
export const approveTravelSegment = async ({
  segmentId,
  userId,
  comment,
}: {
  ...
  if (!updated) throw new Error('Сегмент пропал после обновления');
  return decorateSegmentRow(updated);
};

export const rejectTravelSegment = async ({
  segmentId,
  userId,
  comment,
}: {
...
export const getTravelSegmentEmployeeId = async (segmentId: string): Promise<number | null> => {
  const row = await fetchSegmentRowById(segmentId);
