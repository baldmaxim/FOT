...
describe('checkWeekendWorkRequirement — «Обнулить день»', () => {
  it('обнулённый руководителем выходной (manual/0ч) при СКУД-присутствии не требует служебку', async () => {
    vi.mocked(listEmployeeMembershipsForDepartmentPeriod).mockResolvedValue([
      { employee_id: 1, transferred_out_date: null, joined_date: null },
    ] as never);
    // 10.05.2026 — воскресенье, выходной по графику.
    vi.mocked(resolveSchedulesForPeriod).mockResolvedValue(
      new Map([[1, new Map([['2026-05-10', {} as never]])]]) as never,
    );
    vi.mocked(isWorkingDay).mockReturnValue(false);
    vi.mocked(loadCalendarMonth).mockResolvedValue(null as never);
    pgQuery.mockReset();
    pgQuery
      // 1-й query → attendance_adjustments: «Обнулить день» (status='manual', 0ч).
      .mockResolvedValueOnce([
        { employee_id: 1, work_date: '2026-05-10', status: 'manual', hours_override: 0, source_type: 'manual' },
      ])
      // 2-й query → skud_daily_summary: присутствие 10.05.
      .mockResolvedValueOnce([{ employee_id: 1, date: '2026-05-10', total_minutes: 480 }]);
