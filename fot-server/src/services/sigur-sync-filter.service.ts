...
export async function reconcileDepartmentsActivity(
  ...
  for (const id of sigurDbIds) {
    ...
    if (allowed || protectedByPeople) toActivate.push(id);
  ...
  return { activated, deactivated, assignableChanged, warnings };
}

/**
 * Единственный путь записи фильтра. Одна транзакция: lock → upsert → удаление
 * лишних → реконсиляция активности. Кэши сбрасываются только после коммита.
 *
...
export async function saveSyncFilterWithReconciliation(
  ...
  options: { allowEmpty?: boolean; source: string },
