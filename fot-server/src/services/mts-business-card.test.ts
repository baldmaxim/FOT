...
describe('МТС Бизнес: категоризация строк выписки (parseStatementUsages)', () => {
  ...
  it('parseStatementUsageRows: платёж на ЛС (income/topups) не попадает в строки детализации абонента', () => {
    // Иначе плитка «Прочее» и «Итого» вкладки «Использование» раздуваются
    // на 434 700 ₽ одинаково у всех номеров контракта.
    const stmt = {
      Usages: [
        {
          date: '2026-06-24T03:02:13+03:00',
        ...
        { date: '2026-06-25T10:00:00+03:00', type: 'network', amount: 5, Characteristics: { networkEvent: 'call', factUnitCode: 'SECOND', factUnits: 60, direction: 'O' } },
      ],
    };
    const rows = mtsBusinessCdrService.parseStatementUsageRows(stmt);
    expect(rows).toHaveLength(1);
    expect(rows[0].category).toBe('calls');
    expect(rows.reduce((a, r) => a + r.amount, 0)).toBe(5);
  });
