...
/**
 * Исполнитель SQL: клиент транзакции (withTransaction) либо пул. Позволяет
 * писать функции, которые вызываются и самостоятельно, и внутри транзакции
 * вызывающего кода — критично для расчётов под advisory-lock: все чтения и
 * записи обязаны идти по ОДНОМУ соединению, иначе транзакция не видит
 * собственную незакоммиченную запись, а лишние соединения занимают пул.
 *
 * Сами обёртки «через exec, иначе пул» живут в модулях-потребителях поверх их
 * локально импортированных query/execute — так они остаются мокируемыми в тестах.
 */
