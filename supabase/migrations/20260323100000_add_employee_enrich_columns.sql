-- Добавляем колонки для обогащения данных сотрудников (enrich)
ALTER TABLE public.employees
  ADD COLUMN IF NOT EXISTS tab_number text,
  ADD COLUMN IF NOT EXISTS current_status text,
  ADD COLUMN IF NOT EXISTS permit_expiry_date text,
  ADD COLUMN IF NOT EXISTS registration_cat1 text,
  ADD COLUMN IF NOT EXISTS registration_cat4 text,
  ADD COLUMN IF NOT EXISTS doc_receipt_date text,
  ADD COLUMN IF NOT EXISTS work_object text;

-- VIEW: история событий сотрудника (назначения + зарплата)
